...
if (!empty($_POST)) {
    ...
    if (!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['repeat_password'])) {
        ...
        if (($_POST['password'] == $_POST['repeat_password']) && (strlen($_POST['password']) >= 8)) {
            ...
            // sqlSlect est une varibla qui accueille une chaine de caractère ressemblant à une requête SQL
            $tableUser = mysqli_query($db_connect, $sqlSelect);
            // mysqli_query() est une fonction qui permet à l'aise de la variable $db_connect et de la requete SQL
            // précédante, d'éxecuter la requete SQL (select/insert/update ou delete)

            if ((mysqli_num_rows($tableUser) == 0) && (filter_var($email, FILTER_VALIDATE_EMAIL))) {
                // SI il y a 0 correspondance ALORS l'email est danc bien vier et peut e^tre créer
